const TradeMap = {
    ...
    init() {
        ...
        // Arrow markers for directional flows
        ...
        const arrowColors = {
            ...
            small_value:   "#334155",
            large_weight:  "#facc15",
            medium_weight: "#22c55e",
            small_weight:  "#0f766e",
            large_price:   "#fbbf24",
            medium_price:  "#f8fafc",
            small_price:   "#ef4444",
        };
        Object.entries(arrowColors).forEach(([id, color]) => {
            defs.append("marker")
                .attr("id", `arrow-${id}`)
            ...
            this.zoomBehavior = d3.zoom()
                ...
                .on("zoom", (event) => {
                    ...
                    this.g.selectAll(".trade-arc").attr("stroke-width", function() {
                        const originalWidth = d3.select(this).attr("data-original-width") || 1;
                        return originalWidth / k;
                    });
                    
                    this.g.selectAll(".particle").attr("r", STATE.metric === 'integrated' ? 2.0 / Math.sqrt(k) : 1.8 / Math.sqrt(k));
                    ...
                    this.g.selectAll(".map-label")
                        .attr("font-size", (10 / Math.sqrt(k)) + "px");
                });
    ...
    zoomToRegion(regionName) {
        ...
        // The projection is initially scaled by (width / 6.5) and translated to center.
        // We need to calculate the zoom transform (k, x, y) that centers the region's [lon, lat].
        
        const [targetLon, targetLat] = config.center;
        const targetScale = config.scale;

        // Convert [lon, lat] to projected pixel coordinates [px, py] *at scale 1*
        // NOTE: d3.zoom works on top of the SVG. We need to find where the point is currently,
        // and move it to the center of the screen, while scaling up.
        
        // However, this.projection(coords) gives coordinates based on the projection's internal translation.
        // The simplistic approach for d3.zoomIdentity is:
        // center screen - (projectedPoint * k)
        
        // For d3.geoEquirectangular:
        // The projection function itself returns pixels. 
        // We need to apply the zoom transform to the group element.
        
        // Let's rely on d3.zoomIdentity utilities.
        // We can simulate the transform:
        // If we want the point P (projected) to be at Center C with scale K:
        // C = P * K + T  =>  T = C - P * K
    ...
    renderFlows() {
        ...
        if (!this.g) {
            console.warn('TradeMap not initialized yet, skipping renderFlows');
            return;
        }

        let flows = STATE.filteredData;
        
        // Remove long-distance wrapping lines in 2D
        ...
        const widthScale = d3.scaleSqrt().domain([0, maxValue]).range([0.3, 2.0]);

        // Get current Zoom Level for initial styling
        let currentK = 1;
        ...
        // Helper: get stroke color for a flow
        const getStrokeColor = (d, suffix) => {
            if (isIntegrated) {
                ...
                const key = val >= thresholds.large ? "large" : val >= thresholds.medium ? "medium" : "small";
                const colorKey = `${key}_${suffix}`;
                return STATE.showArrows
                    ? (arrowColors?.[colorKey] || colors[key])
                    : colors[key];
            }
        };

        const getArrowMarker = (d) => {
            if (!STATE.showArrows) return null;
            const metric = isIntegrated ? "value" : STATE.metric === "unit_price" ? "price" : STATE.metric;
            const val = d[metricKey];
            const size = (!isIntegrated && thresholds)
                ? (val >= thresholds.large ? "large" : val >= thresholds.medium ? "medium" : "small")
                : (d.value >= CONFIG.thresholds.value[STATE.hsCode].large ? "large"
                    : d.value >= CONFIG.thresholds.value[STATE.hsCode].medium ? "medium" : "small");
            const suffix = isIntegrated ? "value"
                : STATE.metric === "unit_price" ? "price"
                : STATE.metric;
            return `url(#arrow-${size}_${suffix})`;
        };
        ...
        flowGroup.selectAll(".trade-arc")
            ...
            .attr("d", d => {
                ...
                if (STATE.clockwiseCurve) {
                    return this.createClockwisePath(source, target, d.exporter, d.importer);
                }
                return this.path({type: "LineString", coordinates: [source, target]});
            })
            .attr("stroke", d => {
                if (isIntegrated) {
                    const th = CONFIG.thresholds.value[STATE.hsCode];
            ...
            .attr("stroke-width", d => widthScale(d.value) / currentK)
        ...
        topFlows.forEach(d => {
            ...
            const pathString = STATE.clockwiseCurve
                ? this.createClockwisePath(source, target, d.exporter, d.importer)
                : this.path({type: "LineString", coordinates: [source, target]});
            ...
            for (let i = 0; i < count; i++) {
                const particle = flowGroup.append("circle")
                    .attr("class", "particle")
                    .attr("r", (isIntegrated ? 2.0 : 1.8) / Math.sqrt(currentK))
                    .attr("fill", pColor)
                    .attr("opacity", 0);
        ...
        const activeImporters = [...new Set(flows.map(d => d.importer))];
        const importerSet = new Set(activeImporters);

        // All countries needing a node (exporters + importer-only)
        const onlyImporters = activeImporters.filter(d => !new Set(activeExporters).has(d));
        const allNodeCountries = [...new Set([...activeExporters, ...activeImporters])];

        // All nodes: white (particle color handles distinction)
        flowGroup.selectAll(".country-node")
            .data(allNodeCountries)
            .enter().append("circle")
        ...
        // Exporter labels: white
        flowGroup.selectAll(".map-label-exp")
            .data(activeExporters)
            .enter().append("text")
            .attr("class", "map-label map-label-exp")
            .attr("x", d => this.getProjectedPoint(d)[0] + 6)
            .attr("y", d => {
                // If also an importer, shift up slightly to make room for amber label
                const y = this.getProjectedPoint(d)[1];
                return importerSet.has(d) ? y - 2 : y + 4;
            })
            .text(d => STATE.countryNames[d] || d)
            .attr("fill", "#e2e8f0")
            .attr("font-size", (10 / Math.sqrt(currentK)) + "px")
            .attr("display", d => {
                if (!STATE.showExporterLabels) return "none";
                return this.isVisible(d) ? "block" : "none";
            });

        // ALL importer labels: amber (includes countries that are both)
        flowGroup.selectAll(".map-label-imp")
            .data(activeImporters)
            .enter().append("text")
            .attr("class", "map-label map-label-imp")
            .attr("x", d => this.getProjectedPoint(d)[0] + 6)
            .attr("y", d => {
                // If also an exporter, shift down to avoid overlap
                const y = this.getProjectedPoint(d)[1];
                const exporterSet = new Set(activeExporters);
                return exporterSet.has(d) ? y + 10 : y + 4;
            })
            .text(d => STATE.countryNames[d] || d)
            .attr("fill", "#f59e0b")
            .attr("font-size", (10 / Math.sqrt(currentK)) + "px")
            .attr("display", d => {
                if (!STATE.showImporterLabels) return "none";
                return this.isVisible(d) ? "block" : "none";
            });
    ...
    // Bidirectional curve separation: A→B curves up, B→A curves down
    createClockwisePath(source, target, exporterCode, importerCode) {
        const p1 = this.projection(source);
        const p2 = this.projection(target);
        ...
        if (distance < 10) return `M ${x1},${y1} L ${x2},${y2}`;

        // Country code comparison determines curve direction
        // so A→B and B→A always curve in opposite directions
        const curveUp = exporterCode < importerCode;
        ...
        const offset = Math.max(20, Math.min(100, distance * 0.12));
        const dir = curveUp ? 1 : -1;

        const cx = mx + normPerpX * offset * dir;
        const cy = my + normPerpY * offset * dir;
    ...
    renderLegend() {
        ...
        nodeDiv.className = 'mt-3 pt-2 border-t border-slate-700';
        nodeDiv.innerHTML = `
            <div class="text-[10px] text-slate-500 font-bold uppercase mb-1 tracking-wider">Labels</div>
            <div class="space-y-1">
                <div class="flex items-center gap-2 text-[10px]">
                    <span class="text-slate-200 font-mono">◼</span>
                    <span class="text-slate-300">Exporter (white)</span>
                </div>
                <div class="flex items-center gap-2 text-[10px]">
                    <span class="text-[#f59e0b] font-mono">◼</span>
                    <span class="text-slate-300">Importer (amber)</span>
                </div>
                <div class="flex items-center gap-2 text-[10px] pl-4">
                    <span class="text-slate-400 text-[9px]">Both = 2 labels</span>
                </div>
            </div>`;
